...
namespace Mercurial
{
    ...
    {
        /// <summary>
        /// Indicates whether it is running on Windows or not.
        /// </summary>
        /// <value>
        /// True on Windows, false on elsewhere.
        /// </value>
        ...
        /// <summary>
        /// Gets the mercurial executable.
        /// </summary>
        /// <value>
        /// The mercurial executable.
        /// </value>
        public static string MercurialExecutable
        {
            get { return IsWindows ? "hg.exe" : "hg"; }
        }

        /// <summary>
        /// Gets the platform environment separator.
        /// </summary>
        /// <value>
        /// The platform environment separator.
        /// </value>
        public static char PlatformEnvironmentPathSeparator
        {
        ...
        // FIXME: current implementation could generate garbage for UTF8 BOMs. But 
        // CommandServerOutputDecoder.DecodeOneBlock() hangs when we use UTF8.
        //static readonly Encoding text_encoding = IsWindows ? Encoding.GetEncoding ("iso-8859-1") : new UTF8Encoding (true, false);
        static readonly Encoding text_encoding = Encoding.GetEncoding ("iso-8859-1");
		
        public static string TextEncodingName
        {
            get { return text_encoding.BodyName; }
        }
        ...
        public static IEnumerable<Version> SupportedVersions
        {
            get
            {
                ...
                yield return new Version(1, 9, 1, 0);
                yield return new Version(1, 9, 2, 0);
                yield return new Version(1, 9, 3, 0);
                yield return new Version(2, 0, 0, 0);
                yield return new Version(2, 0, 1, 0);
                yield return new Version(2, 0, 2, 0);
                yield return new Version(2, 1, 0, 0);
            }
        }
        ...
        public static void SetClientPath(string clientPath)
        {
            ...
            if (Directory.Exists(clientPath) && File.Exists(Path.Combine(clientPath, MercurialExecutable)))
                clientPath = Path.Combine(clientPath, MercurialExecutable);
        ...
        private static string LocateClient()
        {
            ...
            return (from path in environmentPath.Split(PlatformEnvironmentPathSeparator)
                    where !StringEx.IsNullOrWhiteSpace(path)
                    let executablePath = Path.Combine(path.Trim(), MercurialExecutable)
